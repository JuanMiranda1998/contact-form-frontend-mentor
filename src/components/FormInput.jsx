...
const FormInput = ({ id, name, formData, handleChange, type, errors, errorActive, formTriggered  }) => {
    ...
    return (
        <div className="w-full flex flex-col justify-center items-start mb-4 lg:mb-0 lg:mr-2">
            <label className="mb-2" htmlFor={id}>{name} <span className="text-[#0c7d69]">*</span></label>
            <input className='w-full py-2 pl-4 border border-[#2b4246] rounded-md cursor-pointer hover:border-[#0c7d69] focus:outline-none focus:border-2 focus:border-[#0c7d69]' type={type} id={id} aria-invalid={isInvalid} name={id} onChange={handleChange} value={formData} onBlur={handleFocus} focused={focused.toString()} />
            <div className="h-6" aria-live='assertive'>
